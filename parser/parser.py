...
def del_mistakes(url):
    import pandas as pd
    df = pd.read_csv(url, skiprows=1, names=['title','original','genres','keywords', 'timestamp','country', 'rating','director','img', 'cast', 'description'])
    
    for i in range(len(df['timestamp'])):
        
        try:
            tmp = int(df['timestamp'][i])
        except:
            df['country'][i] = df['timestamp'][i]
            df['timestamp'][i] = 2018
        
    df.to_csv('movies_data.csv', encoding='utf8')

del_mistakes('movies.csv')